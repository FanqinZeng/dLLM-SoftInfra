...
import time
import types
from .generation_utils import Fast_dLLM_QwenForCausalLM
...
class Fast_dLLM_v2EvalHarness(LM):
    def __init__(
        ...
        **kwargs,
        ...
        self.model.mdm_sample = types.MethodType(Fast_dLLM_QwenForCausalLM.batch_sample, self.model)
...
class Fast_dLLM_v2Eval(EvalMDLM):
    def __init__(self,cfg,**kwargs):
        super().__init__(cfg,**kwargs)
        self.model.mdm_sample = types.MethodType(Fast_dLLM_QwenForCausalLM.batch_sample, self.model)
        self.show_speed=cfg.show_speed
        self.max_new_tokens=cfg.max_new_tokens
        self.batch_size=cfg.batch_size
        self.mask_id=cfg.generation.mask_token_id
        self.use_block_cache=cfg.use_block_cache
        self.small_block_size=cfg.small_block_size
        self.threshold=cfg.generation.threshold
        self.bd_size=cfg.bd_size
    def _encode_pair(self, context, continuation):
        whole_enc = self.tokenizer(context + continuation)["input_ids"]
        context_enc = self.tokenizer(context)["input_ids"]
